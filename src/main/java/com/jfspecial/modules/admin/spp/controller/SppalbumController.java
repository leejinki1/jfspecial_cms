 ...
 *
 */
@ControllerBind(controllerKey = "/admin/sppalbum")
public class SppalbumController extends BaseProjectController {

	private static final String path = "/pages/admin/sppalbum/sppalbum_";

	public void list() {
		TbSppAlbum model = getModelByAttr(TbSppAlbum.class);

		SQLUtils sql = new SQLUtils(" from tb_spp_album t "
				+ " left join tb_spp_album f  on f.id = t.parent_id  where 1=1 ");
		...
		Page<TbSppAlbum> page = TbSppAlbum.dao.paginate(getPaginator(), "select t.*,f.name as parentName ", //
	...
	public void view() {
		TbSppAlbum model = TbSppAlbum.dao.findById(getParaToInt());
		
		TbSppAlbum album = TbSppAlbum.dao.findById(model.getParentId());
	...
	public void del() {
		int id = getParaToInt();
		TbSpp imag = TbSpp.dao.findFirstByWhere(" where album_id = ? ", id);
		if (imag != null) {
	...
	public void save() {
		...
		TbSppAlbum model = getModel(TbSppAlbum.class);
	...
	private String selectParentFolder(Integer selected, Integer id) {
		List<TbSppAlbum> list = TbSppAlbum.dao.find(" select id,name from tb_spp_album " //
