...
import com.jfspecial.modules.admin.spp.model.TbSppTags;
 ...
 * 图片
 */
@ControllerBind(controllerKey = "/admin/spp")
public class SppController extends BaseProjectController {
	...
	public void list() {
		...
		Page<TbSpp> page = TbSpp.dao.paginate(getPaginator(), "select t.* ", //
				sql.toString().toString());

		// 下拉框
		setAttr("selectAlbum", new SppAlbumService().selectAlbum(model.getAlbumId()));
	...
	public void add() {
		// 获取页面信息,设置目录传入
		TbSpp attr = getModel(TbSpp.class);
		setAttr("model", attr);

		// 查询下拉框
		setAttr("selectAlbum", new SppAlbumService().selectAlbum(attr.getAlbumId()));
	...
	public void view() {
		...
		String tags = Db.findFirst("select group_concat(tagname) tags " //
				+ " from tb_spp_tags where image_id = ? order by id", model.getInt("id")).getStr("tags");
		setAttr("tags", tags);
	...
	public void save() {
		...
		TbSpp model = getModel(TbSpp.class);
		...
		// 保存tags
		if (pid != null && pid > 0) { 
			Db.update(" delete from tb_spp_tags where image_id = ?", model.getInt("id"));
		}
