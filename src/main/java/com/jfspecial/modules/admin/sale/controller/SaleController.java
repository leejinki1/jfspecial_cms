 ...
 * 图片
 * 
 * 2018年12月5日 上午11:23:24 ljk
 */
@ControllerBind(controllerKey = "/admin/sale")
public class SaleController extends BaseProjectController {
	...
	public void view() {
		...
		String tags = Db.findFirst("select group_concat(tagname) tags " //
				+ " from tb_sale_tags where image_id = ? order by id", model.getInt("id")).getStr("tags");
		setAttr("tags", tags);
	...
	public void save() {
		...
		// 保存tags
		if (pid != null && pid > 0) { 
			Db.update(" delete from tb_sale_tags where image_id = ?", model.getInt("id"));
		}
