...
public class ProjectAppalbumController extends BaseProjectController {
	...
	public void addAlbum() {
		...
		String sql1="select max(id) id from tb_projectapp_album t";
		model=TbProjectAppAlbum.dao.findFirst(sql1);
	...
	public void delAlbum() {
		...
		TbProjectAppAlbum model = TbProjectAppAlbum.dao.findById(pid);//从数据库获取信息

		//判断分类下是否已有文章或内容
		String sql="select * from tb_projectapp t where album_id="+pid;
		List<TbProjectApp> makers=TbProjectApp.dao.find(sql);
		if(makers.isEmpty()){
			//保存数据库
			model.delete();
			/*ajax返回成功信号*/
