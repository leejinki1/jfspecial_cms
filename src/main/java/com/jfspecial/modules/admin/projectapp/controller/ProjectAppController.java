...
import com.jfspecial.modules.admin.projectapp.model.TbProjectAppTags;
 ...
 * 图片
 */
@ControllerBind(controllerKey = "/admin/projectapp")
public class ProjectAppController extends BaseProjectController {
	...
	public void list() {
		...
		Page<TbProjectApp> page = TbProjectApp.dao.paginate(getPaginator(), "select t.* ", //
				sql.toString().toString());

		// 下拉框
		setAttr("selectAlbum", new ProjectAppAlbumService().selectAlbum(model.getAlbumId()));
	...
	public void add() {
		// 获取页面信息,设置目录传入
		TbProjectApp attr = getModel(TbProjectApp.class);
		setAttr("model", attr);

		// 查询下拉框
		setAttr("selectAlbum", new ProjectAppAlbumService().selectAlbum(attr.getAlbumId()));
	...
	public void view() {
		...
		String tags = Db.findFirst("select group_concat(tagname) tags " //
				+ " from tb_projectapp_tags where image_id = ? order by id", model.getInt("id")).getStr("tags");
		setAttr("tags", tags);
	...
	public void delete() {
		// 日志添加
		TbProjectApp model = new TbProjectApp();
	...
	public void save() {
		...
		// 保存tags
		if (pid != null && pid > 0) { 
			Db.update(" delete from tb_projectapp_tags where image_id = ?", model.getInt("id"));
		}
		...
		if (StrUtils.isNotEmpty(tags)) {
			...
			for (int i = 0; i < tagsArr.length; i++) {
				...
				TbProjectAppTags tag = new TbProjectAppTags();
