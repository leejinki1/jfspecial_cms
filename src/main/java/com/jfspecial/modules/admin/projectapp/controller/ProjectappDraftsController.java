...
public class ProjectappDraftsController extends BaseProjectController {
	...
	private static final String path = "/pages/admin/projectapp/projectapp_drafts";

	//显示保存的草稿
	public void index() {
		String sql = "select t.id,t.name,t.publish_user, t.update_time from tb_projectapp t where  status = 1 and is_drafts = 1 order by sort,id desc";
