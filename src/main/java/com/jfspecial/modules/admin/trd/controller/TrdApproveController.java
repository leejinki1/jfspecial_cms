...
public class TrdApproveController extends BaseController {
    ...
    public void  passArticle() {
        ...
        if(usertype>0&&usertype<10) {
            //修改审核状态
            model.set("approve_status", ArticleConstant.APPROVE_STATUS_PASS); // 需要审核改为update;通过审核为pass;不通过为reject
    ...
    public void  rejectArticle() {
        ...
        if(usertype>0&&usertype<10){
            //1-9是系统管理员
            //修改审核状态
            model.set("approve_status", ArticleConstant.APPROVE_STATUS_REJECT); // 需要审核改为update;通过审核为pass
    ...
    public void  approveArticle() {
        ...
        TbTrd model = TbTrd.dao.findById(pid);
        //判断权限
