...
public class TrdalbumController extends BaseProjectController {
	...
	public void addAlbum() {
		...
		String sql1="select max(id) id from tb_trd_album t";
		model=TbTrdAlbum.dao.findFirst(sql1);
	...
	public void delAlbum() {
		...
		TbTrdAlbum model = TbTrdAlbum.dao.findById(pid);//从数据库获取信息

		//判断分类下是否已有文章或内容
		String sql="select * from tb_trd t where album_id="+pid;
		List<TbTrd> makers=TbTrd.dao.find(sql);
		if(makers.isEmpty()){
			//保存数据库
			model.delete();
			/*ajax返回成功信号*/
