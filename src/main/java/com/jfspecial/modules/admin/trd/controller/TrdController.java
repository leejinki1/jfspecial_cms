...
import com.jfspecial.modules.admin.trd.model.TbTrdTags;
 ...
 * 图片
 */
@ControllerBind(controllerKey = "/admin/trd")
public class TrdController extends BaseProjectController {
	...
	public void list() {
		...
		Page<TbTrd> page = TbTrd.dao.paginate(getPaginator(), "select t.* ", //
				sql.toString().toString());

		// 下拉框
		setAttr("selectAlbum", new TrdAlbumService().selectAlbum(model.getAlbumId()));
	...
	public void add() {
		// 获取页面信息,设置目录传入
		TbTrd attr = getModel(TbTrd.class);
		setAttr("model", attr);

		// 查询下拉框
		setAttr("selectAlbum", new TrdAlbumService().selectAlbum(attr.getAlbumId()));
	...
	public void view() {
		...
		String tags = Db.findFirst("select group_concat(tagname) tags " //
				+ " from tb_trd_tags where image_id = ? order by id", model.getInt("id")).getStr("tags");
		setAttr("tags", tags);
	...
	public void save() {
		...
		// 保存tags
		if (pid != null && pid > 0) { 
			Db.update(" delete from tb_trd_tags where image_id = ?", model.getInt("id"));
		}
