...
public class MakeralbumController extends BaseProjectController {
	...
	public void addAlbum() {
		...
		String sql1="select max(id) id from tb_maker_album t";
		model=TbMakerAlbum.dao.findFirst(sql1);
	...
	public void delAlbum() {
		...
		TbMakerAlbum model = TbMakerAlbum.dao.findById(pid);//从数据库获取信息

		//判断分类下是否已有文章或内容
		String sql="select * from tb_maker t where album_id="+pid;
		List<TbMaker> makers=TbMaker.dao.find(sql);
		if(makers.isEmpty()){
			//保存数据库
			model.delete();
			/*ajax返回成功信号*/
