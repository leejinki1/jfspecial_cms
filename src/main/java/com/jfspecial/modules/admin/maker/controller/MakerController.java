...
import com.jfspecial.modules.admin.maker.model.TbMakerTags;
 ...
 *
 */
@ControllerBind(controllerKey = "/admin/maker")
public class MakerController extends BaseProjectController {
	...
	public void list() {
		...
		Page<TbMaker> page = TbMaker.dao.paginate(getPaginator(), "select t.* ", //
				sql.toString().toString());

		// 下拉框
		setAttr("selectAlbum", new MakerAlbumService().selectAlbum(model.getAlbumId()));
	...
	public void add() {
		// 获取页面信息,设置目录传入
		TbMaker attr = getModel(TbMaker.class);
		setAttr("model", attr);

		// 查询下拉框
		setAttr("selectAlbum", new MakerAlbumService().selectAlbum(attr.getAlbumId()));
	...
	public void view() {
		...
		String tags = Db.findFirst("select group_concat(tagname) tags " //
				+ " from tb_maker_tags where image_id = ? order by id", model.getInt("id")).getStr("tags");
		setAttr("tags", tags);
	...
	public void save() {
		...
		// 图片附件
		if (uploadImage != null) {
			String fileUrl = uploadHandler(site, uploadImage.getFile(), "maker");
		...
		// 保存tags
		if (pid != null && pid > 0) { 
			Db.update(" delete from tb_maker_tags where image_id = ?", model.getInt("id"));
		}
		...
		if (StrUtils.isNotEmpty(tags)) {
			...
			for (int i = 0; i < tagsArr.length; i++) {
				...
				TbMakerTags tag = new TbMakerTags();
