...
public class PolicyalbumController extends BaseProjectController {
	...
	public void addAlbum() {
		...
		String sql1="select max(id) id from tb_policy_album t";
		model=TbPolicyAlbum.dao.findFirst(sql1);
	...
	public void delAlbum() {
		...
		TbPolicyAlbum model = TbPolicyAlbum.dao.findById(pid);//从数据库获取信息

		//判断分类下是否已有文章或内容
		String sql="select * from tb_policy t where album_id="+pid;
		List<TbPolicy> makers=TbPolicy.dao.find(sql);
		if(makers.isEmpty()){
			//保存数据库
			model.delete();
			/*ajax返回成功信号*/
