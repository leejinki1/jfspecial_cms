...
public class PersonController extends BaseProjectController {
	...
	private void common(SysUser user) {
		...
		// title展示
		setAttr(JFSpecialUtils.TITLE_ATTR, "空间 - " + BeeltFunctions.getUserName(user.getUserid()) + " - "
				+ getAttr(JFSpecialUtils.TITLE_ATTR));
	}
	...
	public void saveblog() {
		...
		if (pid != null && pid > 0) { // 更新
			...
			if (model.getFolderId() == null || model.getFolderId() <= 0) {
				model.setFolderId(JFSpecialUtils.MENU_BLOG); // 博文目录
			}
	...
	public void save() {
		...
		if (user.getInt("usertype") != 4) {
			...
			if (!user.getStr("password").equals(JFSpecialUtils.passwordEncrypt(oldPassword))) {
				json.put("msg", "密码错误！");
				renderJson(json.toJSONString());
				return;
			}
			if (StrUtils.isNotEmpty(newPassword) && !newPassword.equals(newPassword2)) {
				...
				model.set("password", JFSpecialUtils.passwordEncrypt(newPassword));
			}
		}
	...
	public void view() {
		...
		// title展示
		setAttr(JFSpecialUtils.TITLE_ATTR, BeeltFunctions.getUserName(userid) + "的空间 - " + getAttr(JFSpecialUtils.TITLE_ATTR));
