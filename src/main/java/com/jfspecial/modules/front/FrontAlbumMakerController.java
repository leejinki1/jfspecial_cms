...
@ControllerBind(controllerKey = "/album/maker")
public class FrontAlbumMakerController extends BaseProjectController {
	...
	public void img() {
		...
		String tags = Db.findFirst("select group_concat(tagname) tags " //
				+ " from tb_maker_tags where image_id = ? order by id", image.getId()).getStr("tags");
		setAttr("tags", tags);
