...
import com.jfspecial.api.constant.ApiConstant;
import com.jfspecial.api.form.ApiForm;
import com.jfspecial.api.form.ApiResp;
import com.jfspecial.api.service.ApiCache;
import com.jfspecial.api.service.IApiLogic;
import com.jfspecial.api.util.ApiUtils;
...
public abstract class BaseApiLogic implements IApiLogic {
	...
	public ApiResp login(ApiForm form) {
		...
		try {
			...
			String sql = " where username = ? and usertype = " + JFSpecialUtils.USER_TYPE_API;
			...
			if (!(user.getInt("usertype") == JFSpecialUtils.USER_TYPE_API)) {
				return new ApiResp(form).setCode(-1004).setMsg("您没有登录权限");
			}

			if (!(user.getInt("state") == JFSpecialUtils.USER_STATE_NORMAL)) {
